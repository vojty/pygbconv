...
def convimg(name):
    ...
    if len(colors) > 4:
        print(
            "Image must contain no more than 4 unique colors. Do not use a file format that would destroy 4 color images such as JPEG. PNG, GIF and BMP (with any color depth) are ey-ok."
        )
        exit()
    elif len(colors) < 4:
        print(
            (
                "Warning! Image has only "
                + str(len(colors))
                + " unique colors (instead of exactly 4.) Color table will amended."
            )
        )
    ...
    if len(colors) == 2:
        ...
        print(
            "Warning: This image only seems to have a single, solid color. How do you expect that to work, cap'n?"
        )
    ...
    pixels_g = [colors_r[x] for x in pixels]
...
def compilerom(gbin, gbout, images):
    gbtiles = list(map(convimg, images))
    f = open(gbin, "rb")
...
def main():
    if len(sys.argv) < 3:
        ...
        print(
            "Actual number of colors in each image must be four or less. Color depth can be anything that PIL can interpret."
        )
        print(
            "Do not use a file format that would destroy 4 color images, such as JPEG. PNG, GIF and BMP (with any color depth) are ey-ok."
        )
        return 0
